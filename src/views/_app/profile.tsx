    ...
    return (
        <div className='container max-w-5xl mx-auto min-h-screen p-10'>
            <div className='flex flex-row items-center justify-around p-10 rounded-lg shadow-accent/40 shadow-lg'>
                ...
                <EditDialog label={'Apelido'} value={user?.username} type={'text'} onChange={() => {}} />
                <EditDialog label={'Email'} value={user?.email} type={'email'} onChange={() => {}} />
                <EditDialog label={'Senha'} value={user?.password ?? '1a2b3c4d'} type={'password'} onChange={() => {}} />
            </div>
            </div>
        </div>
    )
...
const EditDialog = ({label, type, value, onChange}: {label: string, value: string, onChange: string, type: string}) => {
    return <AlertDialog>
        <AlertDialogTrigger asChild>
        <div className='group relative cursor-pointer space-y-2'>
            <Label>{label}</Label>
            <Input type='text' value={value} disabled className='shadow-xsmall transition-all shadow-accent/40 group-hover:-translate-y-[0.5px] group-hover:shadow-md' />
