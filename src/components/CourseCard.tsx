...
export function CourseCard({ course }: { course: Course }) {
  return (
    <Link key={`${course.id}`} to={"/dashboard/course/$id"} params={{ id: course.id! }}>
        <Card className="group cursor-pointer overflow-hidden border-0 bg-card shadow-lg transition-all duration-300 hover:shadow-xl hover:-translate-y-2">
        <div className="relative h-48 overflow-hidden">
            ...
            <div className="absolute bottom-4 left-4 text-4xl">{course.icon && <course.icon />}</div>
        </div>
